...
$(function(){
	...
	*/	
	...
	for(var z = 1; z <= Totales; z++)
	{
		respuestas[z] = $('#Resp'+z).val();
		//alert(respuestas[z]);
	}
	...
	{		
		...
		datos[x].addDropzone('.dropzone'+x, function(e){			
			for(var y = 1; y <= Totales; y++)
			{	
				//alert('hola y = '+y);			
				if(e[0].children[0].id == respuestas[y])
				{
					//alert(e[0].children[0].id);
		...
		/*
		datos[1].addDropzone('.dropzone2', function(e){
			if(e[0].children[0].id == "dos")
			{
				//alert(e[0].children[0].id);
				alert("Orale que buena onda");
			}
		});
		
		datos[2].addDropzone('.dropzone3', function(e){
			if(e[0].children[0].id == "khe kozaz")
			{
				//alert(e[0].children[0].id);
				alert("Hasta que le atinas woooe");
			}
		});
		*/
	}
